...
export class AchievementsScene extends Scene {
  ...
  update(dt, input) {
    ...
    if (!this._checked) {
      ...
      if (newly.length) { this._toastList.push(...newly); this._toastT = 2.5; }
    }

    if (input.justPressed("escape", "backspace") || input.tapIn(this._backBtn())) {
      audio.play("click"); this.game.changeScene(new MenuScene(this.game)); return;
    }
    ...
    // ===== 滚动 =====
    if (input.justPressed("arrowdown", "s") || input.swipeDown) this.scroll += 30;
    if (input.justPressed("arrowup", "w") || input.swipeUp) this.scroll = Math.max(0, this.scroll - 30);

    if (input.pointer && input.pointer.justDown) {
      const tr = this._tabRects();
      for (let i = 0; i < tr.length; i++) {
        if (input.tapIn(tr[i])) { audio.play("click"); this.tab = i; this.scroll = 0; }
      }
    }
  }
  ...
  render(ctx) {
    ...
    g.addColorStop(0, "#160a26"); g.addColorStop(1, "#0d0619");
    ...
    const totalHeight = list.length * (cardH + gap);
    // 限制滚动范围
    this.scroll = Math.max(0, Math.min(this.scroll, totalHeight - (bottom - top) + 20));

    let y = top + 4 - this.scroll;
    for (const a of list) {
      // 裁剪可见卡片
      if (y + cardH < top || y > bottom) { y += cardH + gap; continue; }
    ...
    if (totalHeight > bottom - top) {
      const pct = totalHeight > 0 ? this.scroll / (totalHeight - (bottom - top) + 20) : 0;
      const barH = Math.max(14, (bottom - top) / totalHeight * (bottom - top));
      const barY = top + pct * (bottom - top - barH);
    ...
    if (this._toastT > 0 && this._toastList.length) this._renderToast(ctx, W, H);
  }
